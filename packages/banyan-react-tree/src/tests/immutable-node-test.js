...
describe("Node", () => {
    ...
    it("remove node", () => {
        ...
        expect(info_t2.removed_nodes.map(n => n.id)).to.deep.equal([1, 2, 3]);
        expect(node.toString(t3)).to.equal("n2(n2b)");
        expect(info_t3.changed_nodes.map(n => n.id)).to.deep.equal([4]);
        expect(info_t3.removed_nodes.map(n => n.id)).to.deep.equal([5]);
    });
});
